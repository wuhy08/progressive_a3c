...
class PolicyEstimator():
  ...
  Args:
    dim_inputs: dimension of the observation space
  ...
  def __init__(self, num_outputs, dim_inputs=[84, 84, 4], reuse=False, trainable=True):
    ...
    self.states = tf.placeholder(shape=[None] + dim_inputs, dtype=tf.uint8, name="X")
    # The TD target value
...
class ValueEstimator():
  ...
  def __init__(self, dim_inputs=[84, 84, 4], reuse=False, trainable=True):
    # Placeholders for our input
    # Our input are 4 RGB frames of shape 160, 160 each
    self.dim_inputs = dim_inputs
    self.states = tf.placeholder(shape=[None]+dim_inputs, dtype=tf.uint8, name="X")
