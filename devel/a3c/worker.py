...
class Worker(object):
  ...
  def __init__(self, name, env, policy_net, value_net, global_counter, 
               discount_factor=0.99, summary_writer=None, max_global_steps=None, n_frame = 4):
    self.name = name
    ...
    with tf.variable_scope(name):
      self.policy_net = PolicyEstimator(
        num_outputs = policy_net.num_outputs, 
        dim_inputs = policy_net.dim_inputs
      )
      self.value_net = ValueEstimator(
        reuse=True,
        dim_inputs = value_net.dim_inputs
      )
  ...
  def _reset_env(self):
    frames = []
    frames.append(self.env.reset())
    for i in range(self.n_frame-1):
      action = np.random.randint(self.policy_net.num_outputs)
      frames.append(self.env.step(action)[0])
  ...
  def _step_env(self, action):
    ...
    for i in range(self.n_frame):
      if not done:
        next_frame, next_reward, done, info = self.env.step(action)
      else:
        next_frame = frames[-1]
        next_reward = 0
      frames.append(next_frame)
      rewards += next_reward        
    return np.concatenate(frames, axis=2), rewards, done, info
